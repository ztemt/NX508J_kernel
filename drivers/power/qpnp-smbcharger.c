...
#ifdef CONFIG_ZTEMT_NX508J_CHARGER
#include <linux/qpnp/qpnp-adc.h>
...
int vlowbatt_threshold;
	...
	struct wake_lock		eoc_wake_lock;
 	 ...
 	 bool       bad_charger;
#endif 
#ifdef CONFIG_ZTEMT_NX508J_BATTERY_MONITOR_X
...
enum wake_reason {
	...
	PM_ESR_PULSE = BIT(2),
#ifdef CONFIG_ZTEMT_NX508J_BATTERY_MONITOR_X
	PM_REASON_EOC_CHECK = BIT(3),
#endif
#ifdef CONFIG_ZTEMT_NX508J_BATTERY_MONITOR_X
	PM_REASON_BATTERY_MONITOR = BIT(4),
#endif	
};
...
#ifdef CONFIG_ZTEMT_NX508J_BATTERY_MONITOR_X
static void  check_start_monitor_work(struct smbchg_chip *chip);
...
static bool is_usb_present(struct smbchg_chip *chip)
{
	...
	//DBG_CHARGE("usb register RT_STS reg ---1: %d\n",reg);	
	...
	//DBG_CHARGE("usb register INPUT_STS reg ---2: %d\n",reg);	
...
try_again:
	...
	/*Try 3 Times Or SMB FCC Small Than The Value Of 500mA */
...
/*Setting FCC Again*/
	 ...
	 *
	  Waitting For Return To CC Mode
	 */
...
static void smbchg_parallel_usb_enable(struct smbchg_chip *chip)
{
	 ...
	 */
	 /*
	 chip->parallel.current_max_ma Be Treate As Zero At The First Time
   */
...
typedef enum pm_smbchg_bat_if_low_bat_thresh_type
{
    ...
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_2P8,//4//4
    ...
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_2P88,//9
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P00,
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P10,
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P25,//12
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P35,
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P46,
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_3P58,
    PM_SMBCHG_BAT_IF_LOW_BATTERY_THRESH_INVALID //16
}pm_smbchg_bat_if_low_bat_thresh_type;

//175ms  ; Delay required for battery voltage de-glitch time
#define  PM_REG_CONFIG_SETTLE_DELAY       175  * 1000 
//#define  PM_VLOWBATT_SEC_REG_ADDR   0x012D0
#define  PM_VLOWBATT_REG_ADDR       0xF1
#define VLOWBATT_MASK			SMB_MASK(3, 0)
static int smbchg_low_batt_volt_threshold_set(struct smbchg_chip *chip, int lowbat_threshold)
{
	int rc = 0;
	u8 reg;
	rc = smbchg_sec_masked_write(chip,
			chip->bat_if_base + PM_VLOWBATT_REG_ADDR, VLOWBATT_MASK, lowbat_threshold);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set low batt threshold = %d\n",
									rc);
		return rc;
	}
	else{
	...
	rc = smbchg_read(chip, &reg, chip->bat_if_base + PM_VLOWBATT_REG_ADDR, 1);
	if (rc) {
		dev_err(chip->dev, "Unable to read low batt threshold : %d\n", rc);
		return rc;
	}
 else{
 	 DBG_CHARGE("low batt threshold = %d!\n", reg&VLOWBATT_MASK);
 	}
...
#ifdef CONFIG_ZTEMT_NX508J_BATTERY_MONITOR_X
int zte_smbchg_charging_en(struct smbchg_chip *chip, bool en)
{
   	 smbchg_usb_suspend(chip,false);
   	 smbchg_dc_suspend(chip,false);
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
  ...
  if(chip->bad_charger){
	...
	if (current_limit != chip->usb_target_current_ma) {
		...
		//Result in Re-Calculate AICL & Current Re-Assign
		rc = smbchg_set_thermal_limited_usb_current_max(chip,
				current_limit);
		if (rc < 0)
			dev_err(chip->dev,
...
#ifdef CONFIG_ZTEMT_NX510J_CHARGER
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (chip->usb_psy) {
	  ...
	  if(usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP){
	 	 ...
	 	 DBG_CHARGE("inserted %s, usb psy type = %d stat_5 = 0x%02x\n",
			usb_type_name, usb_supply_type, reg); 
	#else
		power_supply_set_supply_type(chip->usb_psy, usb_supply_type);
	#endif
...
static irqreturn_t aicl_done_handler(int irq, void *_chip)
{
	...
	if(usb_present && bad_charger && !chip->bad_charger){
		chip->bad_charger =  bad_charger;
			...
			chip->usb_target_current_ma =
	 	       min(chip->usb_target_current_ma,SMGCHARGE_IDEV_CHG_MAX);
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	 ...
	 */
	#ifdef CONFIG_ZTEMT_NX508J_CHARGER_X
	#else
...
#ifdef CONFIG_ZTEMT_NX508J_CHARGER
	/* set the Vlowbatt Threshold */
	if (chip->vlowbatt_threshold != -EINVAL) {
		rc = smbchg_low_batt_volt_threshold_set(chip, chip->vlowbatt_threshold);
		if (rc < 0) {
			dev_err(chip->dev,
				"Couldn't set low batt threshold rc = %d\n", rc);
			return rc;
		}
		pr_smb(PR_STATUS, "set vlowbatt threshold to %d\n", chip->vlowbatt_threshold);
	}
	else{
		pr_smb(PR_STATUS, "set vlowbatt threshold failed! \n"); 
	}
#endif
...
#ifdef	CONFIG_ZTEMT_NX508J_CHARGER
	OF_PROP_READ(chip, chip->vlowbatt_threshold, "lowbatt-voltage-threshold", rc, 1);
#endif
...
batt_monitor_worker(struct work_struct *work)
{
   int batt_temperature;
	struct delayed_work *dwork = to_delayed_work(work);
	struct smbchg_chip *chip = container_of(dwork,
				struct smbchg_chip, battery_monitor_work );
	int rc = 0;
	...
	DBG_CHARGE("is_chg_batt_temp_abnormal:%d!\n",
	...
	*/
	if( batt_temperature > CHG_TEMP_LOW2 &&
	  	 ...
	  	 is_chg_batt_temp_abnormal() ){
       rc = zte_smbchg_charging_en(chip,true);
				if(!rc)
				{
					set_chg_batt_temp_st(BATT_TEMP_GOOD);
					power_supply_changed(&chip->batt_psy);
				  ...
				  DBG_CHARGE("set battery normal failed!\n" );
				}
	}else if( (batt_temperature > CHG_TEMP_HIGH1 || 
	                batt_temperature < CHG_TEMP_LOW1) &&
	              (!is_chg_batt_temp_abnormal() )){
			rc = zte_smbchg_charging_en(chip,false);
			if(!rc)
			{
	      set_chg_batt_temp_st(BATT_TEMP_ABNORMAL);
			 ...
			 DBG_CHARGE("set battery abnormal failed ! \n" );
			}	
	}
	...
	return ;
	out_work:
	DBG_CHARGE("Stop Battery Monitor ! \n");
	set_chg_batt_temp_st(BATT_TEMP_GOOD);
	smbchg_relax(chip,PM_REASON_BATTERY_MONITOR);
}
...
*/
static void  
check_start_monitor_work(struct smbchg_chip *chip)
{
//设置充电器状态
   if( chip->usb_present  ) {
	...
	if(is_charger_online()){
				smbchg_stay_awake(chip,PM_REASON_BATTERY_MONITOR);
        schedule_delayed_work(&chip->battery_monitor_work,
					  round_jiffies_relative(msecs_to_jiffies
								(CHG_MONITOR_PERIOD_MS)));
	}
