...
#define DBG_FG(x...) do {if (debug_mask_fg) pr_info(">>ZTEMT_CHARGE>>  " x); } while (0)
#else 
static int debug_mask_fg = 0;
static int  debug_mask_capacity = 0;
#endif
...
wait:
...
#ifdef CONFIG_ZTEMT_NX508J_CHARGER
	if ((debug_mask_fg) && !profiles_same){
		pr_info("charger :jing profiles differ\n");
	}
	if(debug_mask_fg) {
	...
	#ifdef  CONFIG_ZTEMT_NX508J_CHARGER
	DBG_FG(" Battery Data Load Successfully!\n");
	#endif
	return rc;
unlock_and_fail:
	mutex_unlock(&chip->rw_lock);
